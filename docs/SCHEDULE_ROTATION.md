...
1. `docus`：精二但书是全部金单贸易站的第一核心；252 等存在空二级金单贸易站时优先进二级站，再从所有可用干员中按最终效率选择队友。叙拉古不注册固定体系：八幡海铃、伺夜、贝洛内均可不入选，伺夜与贝洛内可单走、同上且跨站、或自然同站；包含八幡海铃的中枢候选只有在最终实际贸易成员使其动态收益胜出时才采用。shortcut 仅按最终实际同房组合自然结算。
...
暖机干员的“房间稳定”只约束其实际连续上岗的多个班次，不构成最低进编次数或固定上岗班次要求。若八幡海铃与分居两个贸易站的实际叙拉古消费者形成同上同下 bind，房间级 αβγ 轮换必须把两站归入同一 cohort；在双贸易站布局中，γ 只能在该 cohort 休息的一班同时接替两站，因此巫恋 + 龙舌兰可能只自然入选这一班。此时不能为了满足旧的“至少两班巫恋”测试而拆开叙拉古 bind；只要巫恋组实际出现时不跨房换站，就满足暖机稳定不变量。
...
| `syracusa_actual` | 最终实际入选且具备家族认可的八幡海铃 E2 + 伺夜/贝洛内贸易成员 | 仅有效 producer 与至少一名实际 consumer 同时存在时派生；同队、上 2 休 1 | `AssignmentPlan::derive_actual_shift_binds` |
