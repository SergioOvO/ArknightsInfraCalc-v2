...
beta 前端当前不调用 `layout eval`。它在 `src/server/infra.ts` 中向长驻 `infra-cli serve` 发送 `plan`，再从 `result.shifts[].efficiencies.room_lines` 提取贸易、制造和发电结果。`src/types.ts` 的 `RotationShift` 也只声明这三个生产域。因此核心提交 `b822c82` 不会破坏当前前端，但新结果也不会自动出现在页面。

办公室与会客室的收益量纲不同于贸易、制造和发电：办公室速度影响公招刷新次数获取，会客室速度影响线索搜集。当前模型只输出技能速度加成，不输出设施基础速度、实际公招券/黄票收益、线索概率或信用收益。前端不得把这些值加进现有生产总分。

## 决策

### 1. 复用现有 `serve plan` 请求

前端继续使用现有 `/api/plan` 请求和 `infra-cli serve` 的 `plan` 方法，不新增第二个求解请求，也不要求浏览器提交显式 assignment。

核心在每个班次的 shift 对象中增加可选字段 `support_facilities`：
...
- 新核心始终可以在 shift 中提供 `support_facilities`。
- 旧核心没有该字段；前端必须把它声明为可选。
...
`rotationShiftsFromServe` 当前通过对象展开保留未知 shift 字段，但前端仍应显式规范化 `support_facilities`，拒绝非数组房间列表和非有限数值，避免仅靠 TypeScript 类型断言信任进程输出。
...
4. 为字段存在、旧字段不变、autofill 和 6/12 小时时间爬升增加 serve 实测。
...
- 使用旧核心：`support_facilities` 缺失，前端计划生成与当前一致。
- 使用新核心：每个 shift 可选携带办公室、会客室数组。
...
- 乌有三级办公室向同班会客室注入 10%。
- `serve plan` 的实际 JSON 包含本 ADR 定义的结构。
