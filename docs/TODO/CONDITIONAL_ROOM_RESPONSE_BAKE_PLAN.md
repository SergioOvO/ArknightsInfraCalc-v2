...
> 交接状态：2026-07-17 已完成真实技能只读提取；下一步由用户确认首批机制组合，并裁决
> 戴菲恩逐房/跨房语义冲突。确认前不修改 Bake schema，不预设按单房、半区或全基地 Bake。
...
| 机制 | owner / buff | 最小角色集合或观察量 | 响应不同的子集或阈值 | 作用域 / 外部状态 | 普通队友处理 |
|---|---|---|---|---|---|
| 戴菲恩—格拉斯哥 producer | 戴菲恩 `control_tra_limit&spd[010]` + 各房格拉斯哥 workforce | 戴菲恩 presence + 每房 `n_g(r)`；与本房 consumer signature 联合 | canonical：每房 `n_g(r)=0/1/2/3`；当前代码：全贸易总数 | 中枢→贸易；**语义冲突，见下文** | 非格拉斯哥者可 Join；格拉斯哥者更新逐房摘要 |
...
canonical `CONTROL_CENTER_ASSIGNMENT.md` 规定戴菲恩对每间贸易房注入
`10 × n_g(r)`，且明确不同房间不得先求总和。当前 `skill_table.json` 的
`control_tra_limit&spd[010]` 使用 `tagged_count_in_trade_sum(cc.g.glasgow)`；中枢解释器将其
编译为全贸易总数 inject，贸易房再读取同一个总数。两者会产生不同的 response key 和数值，
因此按真源优先级，戴菲恩不能进入首批 Bake，直到实现统一为 canonical 逐房语义，或用户明确
改判 canonical。现有 `vina_lungmen` / `meta_vina` segment 也是文档已确认的实现缺口，不能用来
替代裁决。
...
5. 戴菲恩待逐房/跨房语义统一后再纳入；全局资源链随后作为跨设施验收轴。

上述顺序只用于选择首批验证面，不是 top-K、hard constraint、required admission 或近似剪枝。
本次提取是 analysis-only，不改变候选集合、排序、Plan、rotation 或运行时答案。
