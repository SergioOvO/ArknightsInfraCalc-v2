...
> 本文负责：记录用户意图、项目现状、任务分类、Agent 协作方式和规则降维求解器的开发准则
> 本文不负责：裁决明日方舟业务规则、描述当前代码已经具备的能力，或授权立即修改代码
> 后续用途：先以本文修订根 `AGENTS.md`、项目 Skills 和工作流文档，再决定求解器 assurance 能力的实施顺序
...
不同换班模式共享干员池、游戏机制、单设施计算、组合合法性和心情基础能力，但不能共享未经证明相同的调度假设。当前评审边界见 [排班改版逻辑设计](排班改版逻辑设计_公孙长乐评审.md)。
...
### Phase A：修正 Agent 工作流表达

1. 以本文为设计基线，重写根 `AGENTS.md`；
2. 调整 maintenance、system audit、evidence 三个 Skill 的入口和边界；
3. 同步 `MAINTENANCE_MODE`、`QUALITY_AND_AUDIT`、`INDEX`、`PROJECT_MAP`；
4. 统一 Team / Shift 术语路由；
5. 让 reviewer 检查是否再次偏向“所有问题都小修”或“所有问题都重构”。

这应作为独立 workflow / quality 任务实施，不与业务 bug 混合。
