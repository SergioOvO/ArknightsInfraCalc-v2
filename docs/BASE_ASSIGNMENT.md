...
**贸易 core priority**：精二但书是所有金单贸易站的第一核心；存在空二级金单站时先把该站交给但书，再在所有可用干员中按 `final_efficiency` 选择最高效率队友。叙拉古跨站体系只固定八幡海铃进中枢；伺夜、贝洛内作为两个独立的贸易 `search` 成员，不预设同房，并可自然进入但书站候选。`gsl_docus_solo` / `gsl_docus_syracusa` 只在求解候选组合时提供机制结算，不参与队友候选的优先顺序。迷迭香体系激活时，黑键作为 required trade anchor 保证进编；可露缺黑键仍上可露。自动龙巫必须包含巫恋 + 龙舌兰 + 裁缝 β/α，普通白板只保留单站结算兼容，不进入自动 role；无龙舌兰的巫恋兜底低于推王组。
